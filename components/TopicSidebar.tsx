  ...
  return (
    <aside className="hidden w-64 flex-shrink-0 lg:block">
      <nav className="sticky top-20 space-y-6 overflow-y-auto pb-8 pt-4">
        <h3 className="text-muted-foreground text-xs font-bold uppercase tracking-wider">{currentOutcome}</h3>
        <div className="space-y-3">
          {sections.map((section) => (
            <div key={section.number}>
              <h4 className="text-foreground mb-1 text-sm font-semibold">{section.number} {section.title}</h4>
